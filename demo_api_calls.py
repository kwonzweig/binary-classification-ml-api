...
predict_samples = predict_df.sample(
    n=5, random_state=42
)  # Select 5 samples for prediction
...
data_for_prediction = (
    predict_samples.drop(columns=[target_column])
    .apply(lambda row: row.where(pd.notnull(row), None).tolist(), axis=1)
    .tolist()
)

# Prepare the prediction request without feature names
predict_request = {"data": [{"features": row} for row in data_for_prediction]}
